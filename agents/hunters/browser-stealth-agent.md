...
- **Server did not start**: `$CLAUDE_PROJECT_DIR/tools/camofox_ctl.sh logs` and check for missing `xvfb-run`, corrupt Camoufox binary, or port already bound. Do not retry blindly.
...
When unsure about scope, invoke `uv run python3 $CLAUDE_PROJECT_DIR/tools/scope_check.py <target>` before proceeding.
