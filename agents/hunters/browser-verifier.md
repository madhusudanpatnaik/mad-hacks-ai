...
If CF detected (or ANY target — for consistency) → use camofox-browser (stealth):
```bash
# Health check + lazy start from packs/camofox-browser/
curl -sSf http://localhost:9377/health >/dev/null || \
...
```
Full runtime + REST contract: `packs/camofox-browser/AGENTS.md`. If startup fails: `tail -50 /tmp/camofox.log`.
...
**MANDATORY: Walk the rotation ladder — 4-tier detection method (see `~/.claude/skills/mad-hacks/brain/payloads/xss.txt` + `xss-waf-bypass.txt` for full payload sets per tier).**
