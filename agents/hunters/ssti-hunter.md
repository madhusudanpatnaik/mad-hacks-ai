...
2. **Intelligence router — unified query across every knowledge source:**
   ...
   bash ~/.claude/skills/mad-hacks/scripts/intelligence-recall.sh "<target-relevant-keywords>" \
        --target <target> --class ssti --limit 12
   ```
...
4. **Blind class → MANDATORY OOB attribution** (`ssti` is a blind class — no callback = not confirmed):
   ```bash
   PAYLOAD=$(bash ~/.claude/skills/mad-hacks/scripts/oob.sh seed <target> ssti <param>)  # fresh per param
...
bash scripts/engagement-state.sh exhausted <target> ssti <vector> <variant> "<why>" [evidence]
