...
2. **Intelligence router — unified query across every knowledge source:**
   ...
   bash ~/.claude/skills/mad-hacks/scripts/intelligence-recall.sh "<target-relevant-keywords>" \
        --target <target> --class xss --limit 12
   ```
...
bash scripts/engagement-state.sh exhausted <target> xss <vector> <variant> "<why>" [evidence]
...
```
~/.claude/skills/mad-hacks/references/hunt-xss.md
```

This is the mad-hacks native XSS methodology — public bug-bounty
distillation, 2024-2026 CVE catalog (DOMPurify mXSS family
CVE-2024-47875 / 45801; Auth0 nextjs-auth0 returnTo; React Server
Components family; markdown-to-jsx CVE-2024-21535; Grafana path→XSS
CVE-2025-4123; listmonk admin-ATO chain), the 10 sub-techniques (A-J)
covering reflected/DOM, postMessage, mXSS, OAuth returnTo, prototype
pollution → DOM XSS, stored XSS chains, SVG, Trusted Types bypass,
markdown renderer XSS, RSC / Server Actions / Agentic LLM output
rendering, plus the 7-tier detection ladder, the WAF-profile gate +
bypass ladder, source-code review patterns (Semgrep / ast-grep /
ripgrep / CodeQL), chain templates, and the browser-verify evidence
contract. **This file is the source of truth for XSS testing.** It
points to the context-organized payload index
(`brain/payloads/xss-by-context.md`) and the raw libraries
(`brain/payloads/xss.txt`, `xss-waf-bypass.txt`).

## MANDATORY: Search prior art

After reading the methodology, call:
...
fall back to `~/.claude/skills/mad-hacks/brain/payloads/xss-by-context.md`
— the context-organized payload index (HTML-body, attribute-breakout,
JS-string, URL/href, DOM-sink, SVG, markdown, template-literal,
polyglot, mXSS, encoding/WAF-differential, blind-XSS, short-form
buckets, each with real payloads copied from the raw
`brain/payloads/xss.txt` library and grep recipes to pull more).
...
in `references/hunt-xss.md` (§ "Detection ladder") for every
JS-execution probe:
...
1. Read `~/.claude/skills/mad-hacks/references/hunt-xss.md` § "WAF-profile gate + bypass ladder" — the 7-level ladder is the contract.
2. Read `~/.claude/skills/mad-hacks/brain/payloads/xss-waf-bypass.txt` + `~/.claude/skills/mad-hacks/references/vuln-playbooks.md` (403-bypass section) for the Cloudflare / Akamai / AWS WAF / F5 / Imperva / Sucuri variants.
...
fail, record the WAF profile in brain (`bash
~/.claude/skills/mad-hacks/scripts/brain.sh note <target>
"waf-profile: <waf> <version> — levels attempted, nothing through"`)
and escalate to a different endpoint — but do not claim the endpoint is
"not vulnerable".
...
Apply the matching sub-technique from the methodology (Sub-technique A for
...
for each rung live in `~/.claude/skills/mad-hacks/brain/payloads/xss-by-context.md`;
this is the runtime decision flow.

1. **`<script>` blocked** → switch to event attributes (`<img src=x onerror=...>`, `<svg onload=...>`) or auto-fire HTML5 triggers (xss-by-context.md → HTML-body / attribute-breakout buckets).
...
9. **Stacked encoding for client-side decode** → if the app does `URLSearchParams` + `JSON.parse` + `decodeURIComponent` on input, layer encodings (xss-by-context.md → Encoding & WAF-differential bucket).
...
Before starting, recall brain briefings for EXHAUSTED vectors — skip them:
`bash ~/.claude/skills/mad-hacks/scripts/brain.sh recall <target>`. Focus
on ACTIVE leads. After completing, label every finding CONFIRMED,
POTENTIAL, or EXHAUSTED with attempt counts and failure reasons, and
write them back with `brain.sh finding|note|exhausted <target> "…"`.
...
- Escalate confirmed XSS via the `chain-builder` agent (XSS→ATO, XSS→admin, blind-XSS→internal). See `references/hunt-xss.md` § "Chain templates".
