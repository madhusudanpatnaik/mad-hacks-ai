...
| **Verified / Inferred / Assumed labeling** — doctrine extension for the VERIFY/REFUTE gate. Load before shipping any finding, chain, or severity call. **t3-verifier's card must include a Label-breakdown section (see deadangle.md § Wiring).** | `references/deadangle.md` |
| **XSS surface probe** (missing "J" probe complementing `scripts/surface-probe.sh` C/F/G/H/I) | `scripts/xss-surface.sh` — feed output into `xss-hunter` |
...
## Personal-research + AI-lab packs

- **packs/vulnerability-research/** — skraft9/vulnerability-research. 8 concise cheatsheets (sqli/xss/ssrf/rce/auth-bypass/dangerous-functions/RE/linux), 3 recon scripts, 1 report template, 3 write-ups (incl. CVE-2026-9136 MISP batch-import IDOR + ZVE-2026-3795 UDP signed-underflow). 46 files, 288K. Reference material — cheatsheets, not tools. See brain/lessons.md "BATCH/BULK ENDPOINT AUTH BYPASS", "UDP LISTENER FUZZ", "SSO TRANSLATION GAP", "DNS-REBINDING", "ENV.JS + SHORT-NUMERIC-ID".
- **packs/AILA/** — project-lambda-zero/AILA (39M, 2153 files). AI/LLM attack lab — mostly re-hashes OWASP LLM Top 10 which we already cover via packs/ai-pentesting/. Kept as rehydratable pack for research reference; NOT folded into brain (LLM-focused, keyless-scope-limited). Rehydrate via reinstall-packs.sh if a specific LLM-attack technique is needed.
