...
#
# Runs under 30 seconds on a warm brain. Exit 0 = all green, 1 = any failure.
set -uo pipefail
...
section "1/2  scope adversarial boundary (14 attack variants + side-effect invariants)"
...
echo ""
section "2/2  end-to-end engagement chain (scope → state → recon → recall → evidence → report)"
...
sep
if [ "$fail" -eq 0 ]; then
  echo " ✓ ALL E2E SUITES PASSED  ($pass/2)"
  exit 0
else
  echo " ✗ FAILURES: $fail/2 suites"
  exit 1
fi
